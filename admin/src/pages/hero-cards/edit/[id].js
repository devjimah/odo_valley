...
export default function EditHeroCard() {
  ...
  const fetchHeroCard = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/hero-cards/${id}`);
      setFormData(response.data.data);
      setError("");
    } catch (err) {
      setError(err.response?.data?.message || "Failed to fetch hero card");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/hero-cards/${id}`, formData);
      router.push("/hero-cards");
    } catch (err) {
      setError(err.response?.data?.message || "Failed to update hero card");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Edit Hero Card</h1>
          <p className="text-gray-600">Update the carousel card information</p>
        </div>
              ...
              {formData.image && (
                <div className="mb-4">
                  ...
                  <img
                    ...
                    onError={(e) => {
                      e.target.src =
                        "https://via.placeholder.com/192x144/e5e7eb/9ca3af?text=Invalid+URL";
                    }}
                  />
                </div>
              )}
